...
stu student;
vector <stu> v;
WCHAR lastname[] = L"김이박송최류장정전서한양";
WCHAR firstname[] = L"선빈영정준재유원진하희성은서도지연민환현"; //랜덤 이름
FLOAT cur_h;
FLOAT info_s, info_r;
FLOAT dist;
FLOAT win_height;
...
HRESULT DemoApp::Initialize(HINSTANCE hInstance)
{
	...
	if (SUCCEEDED(hr))
	{
		...
		// 윈도우를 생성함.
		m_hwnd = CreateWindow(
			L"D2DDemoApp", L"Direct2D Demo Application", //이름
...
HRESULT DemoApp::OnRender()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		{
			sort(v.begin(), v.end(), cmp); //sort로 정렬
		...
		D2D1_RECT_F upper_fix_rectangle = D2D1::RectF(  //맨 위에 쌓일 상자
			rtSize.width / 2 - 45.0f, 0,
			rtSize.width / 2 + 35.0f, 40.0f
		);
		tbx.left = rtSize.width / 2 - 50.0f; tbx.top = 0; tbx.right = rtSize.width / 2 + 20.0f; tbx.bottom = 40.0f;
		tbx2.left = rtSize.width / 2 - 60.0f; tbx2.top = cur_h; tbx2.right = rtSize.width / 2 + 80.0f; tbx2.bottom = cur_h + 40.0f;
		// 채워진 사각형을 그림.
		m_pRenderTarget->SetTransform(D2D1::Matrix3x2F::Identity());
...
D2D1_RECT_F DemoApp::MakeChasingRect(D2D_POINT_2F cur, FLOAT h)
{
	...
	if (cur.y < 20) { //20
		...
		beta1 = 1 - 20 * alpha1;  //1-20 //
		beta2 = -20 * alpha2;
		scale_ratio = alpha1 * cur.y + beta1 + 0.9; //
		rotate_degree = (int)(alpha2 * cur.y + beta2) % 360;
	}
	if (scale_ratio > 4)
		scale_ratio = 4.0f;  //4, 4.0f
	////////////////////////////////////////
			...
			case WM_LBUTTONDOWN:
			{
				...
				if (current_p.x >= tbx.left && current_p.x <= tbx.right) {//tbx맨위 사각형을 가리킴.
					if (current_p.y >= tbx.top && current_p.y <= tbx.bottom) {
						if (recNum > 7) {
							MessageBoxW(hwnd, L"상자가 꽉 찼습니다", L"error", 0);
							return 0;
						}
